...
export default function GradientBlinds({
  ...
  variant = 'blinds', // 'blinds' | 'reactbits'
}) {
...
vec3 getColor(float t){
  t = clamp(t, 0.0, 1.0);
  float scaled = t * float(max(1, uColorCount - 1));
  int seg = int(floor(scaled));
  float f = fract(scaled);
  vec3 cols[8];
  cols[0]=uColor0; cols[1]=uColor1; cols[2]=uColor2; cols[3]=uColor3;
  cols[4]=uColor4; cols[5]=uColor5; cols[6]=uColor6; cols[7]=uColor7;
  vec3 a = cols[seg];
  vec3 b = cols[min(seg+1, uColorCount-1)];
...
void main(){
  ...
  float t = iTime * 0.15;
  float bands = sin(p.x * 6.2831 + t*2.0) * 0.5 + 0.5;
  float curl = noise(p*2.0 + vec2(t*0.7, t*0.5));
  float mixT = clamp(bands*0.75 + curl*0.25, 0.0, 1.0);
  if (uMirror > 0.5) {
    mixT = 1.0 - abs(1.0 - 2.0 * mixT);
  }

  vec3 col = getColor(mixT);

  float v = 1.0 - dot(uv-0.5, uv-0.5) * 1.5;
  col *= clamp(v, 0.7, 1.0);
  float grain = (hash(fragCoord + iTime) - 0.5) * uNoise;
  col += grain;

  gl_FragColor = vec4(col, 1.0);
}`

    const fragment = variant === 'reactbits' ? fragmentReactBits : fragmentBlinds
